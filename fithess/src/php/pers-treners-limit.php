<?php
$sql = "SELECT * FROM `treners` join `post` on `treners`.`postCode` = `post`.`postId` join genderTreners on genderTrenersId = genderCode where `postCode`=1 limit 7";
if($result = $link->query($sql)){
    foreach($result as $row){
      echo '<div class="personal-treners-card" data-sex="'.$row["genderTrenersValue"].'">';
